...
def ner_model(sample_conversation):
	...
	convo_nlp = nlp(sample_conversation)
	return convo_nlp
...
def flight_offer_response(response, extracted_location):
	...
	# iata_code = [*response['dictionaries']['locations'].keys()]
	try: 
		iata_code = list(response['dictionaries']['locations'].keys())
	except KeyError:
		pass

	#iata_code = list(response['dictionaries'].get('locations').keys()) #gets a list of airport codes
	...
	try:
		for offer in response['data']:
			...
			response_str = (f'Your flight from {extracted_location[0]} ({iata_code[0]}) to {extracted_location[1]} ({iata_code[1]}) would be with {flight_carrier_str} and cost ${price} CAD, from {depart_time} to {arrival_time}. Is that okay?')
...
def handle_message(event_data):
	...
	if message['user'] != slack_bot_id:
		...
		nlp_doc = ner_model(user_message)
		...
		# Accesses Amadeus with extracted information to look up flight offers
